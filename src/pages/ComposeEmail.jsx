  ...
  return (
    <div className="flex flex-col gap-6 text-left">
      ...
      <div className="w-full">
        {/* Main Compose Form */}
        <form onSubmit={handleSubmit(onSubmit)} className="w-full grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
          
          {/* Left Column: Core Email Content */}
          <Card className="lg:col-span-3 flex flex-col gap-4 h-full">
            {/* Template Chooser */}
            ...
            <Card className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2.5">
                    ...
                    {uploadedFiles.map((file, idx) => (
                      <div
                        ...
                        className={`flex items-center justify-between px-3 py-2 rounded-lg border text-xs font-semibold ${
                          !file.data 
                            ? 'border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400' 
                            : 'border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 text-slate-650 dark:text-slate-350'
                        }`}
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {!file.data && (
                            <span className="text-[8px] font-black bg-red-100 dark:bg-red-900/50 px-1 py-0.5 rounded text-red-700 dark:text-red-300 uppercase tracking-wide">
                              No Data
                            </span>
                          )}
                          <button
                            ...
                            className="text-slate-450 hover:text-danger cursor-pointer"
                          >
                ...
                <FileUpload
                  accept="*"
                  maxSizeMB={25}
                  label="Click or Drag attachments"
                  sublabel="PDF, Image, Video, Audio up to 25MB"
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-200">Schedule Delivery</span>
                  <span className="text-[10px] text-slate-400">Send this email at a later date/time</span>
                </div>
                <input
                  type="checkbox"
              ...
              {/* Schedule Fields */}
              {isScheduleEnabled && (
                <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-[12px] border border-slate-150 dark:border-slate-800">
                  <CalendarPicker
